...
export default function DashboardProducts() {
  let {
    ...
    token,
    setProductToEdit,
    productStateFilterPreference,
  } = useContext(AppContext);
  const { toDisplayProducts } = useMenuProductsDispayer(
    adminSearchQuery,
    products
  );

  let { FilteredProducts } = useProductStateFilter(
    productStateFilterPreference,
    toDisplayProducts
  );
              ...
              <Item key={product._id} item={product}>
                <Fragment>
                  ...
                  <DeleteOfDatabaseButton
                    onClick={() =>
                      deleteProductAPI(productsAPI, token, product._id)
                    }
                  >
